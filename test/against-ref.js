...
test('XX handshake against reference impl', t => {
  ...
  const client = ref.initialize('XX', true, new Uint8Array(0), clone(initiator.s), clone(initiator.e))
  const server = ref.initialize('XX', false, new Uint8Array(0), clone(responder.s), clone(responder.e))
  ...
  const clientTx = new Uint8Array(512)
  const serverRx = new Uint8Array(512)

  const serverTx = new Uint8Array(512)
  const clientRx = new Uint8Array(512)

  // ->
  ref.writeMessage(client, new Uint8Array(0), clientTx)
  ...
  // <-
  ref.writeMessage(server, new Uint8Array(0), serverTx)
  ...
  // ->
  const splitClient = ref.writeMessage(client, new Uint8Array(0), clientTx)
  const splitServer = ref.readMessage(server, clientTx.subarray(0, ref.writeMessage.bytes), serverRx)

  storeHash(client, refHandshakeHashes)
  storeHash(server, refHandshakeHashes)

  t.deepEqual(initiator.rx, new Uint8Array(splitClient.rx.subarray(0, 32)))
  t.deepEqual(initiator.tx, new Uint8Array(splitClient.tx.subarray(0, 32)))
  t.deepEqual(initiator.rx, new Uint8Array(splitServer.tx.subarray(0, 32)))
  t.deepEqual(initiator.tx, new Uint8Array(splitServer.rx.subarray(0, 32)))

  while (handshakeHashes.length && refHandshakeHashes.length) {
    t.same(handshakeHashes.shift(), new Uint8Array(refHandshakeHashes.shift()))
  }
...
test('IK handshake against reference impl', t => {
  ...
  const server = ref.initialize('IK', false, new Uint8Array(0), clone(responder.s), clone(responder.e))
  const client = ref.initialize('IK', true, new Uint8Array(0), clone(initiator.s), clone(initiator.e), clone(responder.s).publicKey)
  ...
  // ->
  const splitClient = ref.writeMessage(server, new Uint8Array(0), serverTx)
...
test('IK handshake with reference server', t => {
  ...
  initiator.initialise(new Uint8Array(0), keypair.publicKey)

  const server = ref.initialize('IK', false, new Uint8Array(0), keypair)
  const serverRx = new Uint8Array(512)
  const serverTx = new Uint8Array(512)
...
test('IK handshake with reference client', t => {
  ...
  const client = ref.initialize('IK', true, new Uint8Array(0), keypair, null, responder.s.publicKey)
  const clientRx = new Uint8Array(512)
  const clientTx = new Uint8Array(512)
  ...
  t.deepEqual(responder.rx, new Uint8Array(splitServer.rx.subarray(0, 32)))
  t.deepEqual(responder.tx, new Uint8Array(splitServer.tx.subarray(0, 32)))
...
test('XX handshake with reference server', t => {
  ...
  const server = ref.initialize('XX', false, new Uint8Array(0), keypair)
  const serverRx = new Uint8Array(512)
  const serverTx = new Uint8Array(512)
  ...
  t.deepEqual(initiator.rx, new Uint8Array(splitClient.tx.subarray(0, 32)))
  t.deepEqual(initiator.tx, new Uint8Array(splitClient.rx.subarray(0, 32)))
...
test('XX handshake with reference client', t => {
  ...
  const client = ref.initialize('XX', true, new Uint8Array(0), keypair)
  const clientTx = new Uint8Array(512)
  const clientRx = new Uint8Array(512)
  ...
  t.deepEqual(responder.rx, new Uint8Array(splitServer.tx.subarray(0, 32)))
  t.deepEqual(responder.tx, new Uint8Array(splitServer.rx.subarray(0, 32)))
...
test('Bugfix: prologue >64 bytes', t => {
  ...
  const prologue = new Uint8Array(65).fill(1)
...
function randomBytes (n) {
  const bytes = new Uint8Array(Math.ceil(Math.random() * n))
